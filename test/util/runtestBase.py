...
class RuntestBase:
    ...
    def getTestsList(self, startDir):
        ...
        for a in [d for d in startDir if isdir(d) and not (basename(d) in self.exclude)]:
            for d, dirs, files in walk(a, followlinks=True):
                tests += [join(d,f) for f in files if self.istest(f)]
    ...
    def compileWithAsh(self, tests):
        ...
        if not pexpect:
              for test in tests:
                  print '%d\tcompiling %s' % (total,test)
            ...
            for test in tests:
                ...
                if not exists(testdir+".abc"):
                    print("ERROR abc file %s.abc not created" % (testdir))
                total -= 1;
    ...
    def printOutput(self,request, outputCalls):
    ...
    def runTests(self, testList):
        ...
        if self.threads == 1 or self.osName == 'win':
            for t in testList:
                testnum -= 1
