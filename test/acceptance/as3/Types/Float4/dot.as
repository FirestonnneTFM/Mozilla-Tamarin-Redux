...
AddTestCase("float4.dot() returns a float", "float", getQualifiedClassName(float4.dot(flt4, flt4)));
AddTestCase("float4.dot() length is 2", 2, float4.dot.length);
AddErrorTest("float4.dot() with no args", ARGUMENTERROR+1063,  function(){ float4.dot(); });
