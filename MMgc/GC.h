...
namespace MMgc
{
    ...
    public:
        ...
        //  GCRoot's GCMemberRef definition.
        //  implementation for GCMemberRef of GCObjects is the same as GCRef
        template <class T, GCObjectType N = T::gcObjectType>
        class GCMemberRef : public GCRef<T>
        {
        public:
            //  Allow Any GCObject::GCMemberRef to be assigned to another GCRef <class T2>
            template <class T2>
            REALLY_INLINE void operator =(const GCRef<T2>& other)
            {
                GCRef<T>::operator=(other);
            }

            //  This constructor takes any other GCRef<T2>
            //  The additional debug parameter is for checking isAddressOnStack()
            template <class T2>
            explicit REALLY_INLINE GCMemberRef(const GCRef<T2> &other)
            : GCRef<T>(DEBUG_ARG(false))
            {
                this->t = ProtectedGetOtherRawPtr(other);
            }
        ...
        //  RCObject specialization
        //  This class needs to call the correct increment/decrement whenever T is an RCObject or RCObject subclass
        template <class T>
        class GCMemberRef<T, kTypeRCObject> : public GCRef<T>
        {
        ...
        private:
            ...
            //  lock this up.  I don't think we'll need it
            GCMemberRef<T, kTypeRCObject>(const GCMemberRef<T,kTypeRCObject> &other);
        ...
        public:
            ...
            REALLY_INLINE GCMemberRef& operator =(const GCMemberRef &other)
            {
                set(ProtectedGetOtherRawPtr(other));
                return *this;
            }

            //  The additional debug parameter is for checking isAddressOnStack() Assert
            GCMemberRef() : GCRef<T>(DEBUG_ARG(false)){}
            //  This constructor takes any other GCRef<T2>
            ...
            //  Make sure to decrement the refcount on RCObjects when this GCMemberRef is destroyed
            ~GCMemberRef()
            {
                set(NULL);
            }
