...
##
if [ ! -e "$buildsdir/$change-${changeid}/$platform/$shell_release_nojit" ]; then
    echo "Download AVMSHELL"
    ../all/util-download.sh $vmbuilds/$branch/$change-${changeid}/$platform/$shell_release_nojit $buildsdir/$change-${changeid}/$platform/$shell_release_nojit
    ret=$?
    test "$ret" = "0" || {
        echo "Downloading of $shell_release_nojit failed"
        rm -f $buildsdir/$change-${changeid}/$platform/$shell_release_nojit
        exit 1
    }
    chmod +x $buildsdir/$change-${changeid}/$platform/$shell_release_nojit
fi
...
export AVM=$buildsdir/$change-${changeid}/$platform/$shell_release_nojit
